...
def run(app, host, port, reload, env_vars_path=None):
    ...
    print(
        f"Starting server {app} with uvicorn on {host} - port {port} - reload={reload}"
    )
...
def update_env_vars_from_yml(yml_path):
    ...
    with open(yml_path, "r") as f:
        ...
        if len(env) == 0:
            msg = f"WARNING: Could not find env_variables in {yml_path}, not overriding environment variables"
            print(msg)
