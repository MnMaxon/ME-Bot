...
class MEMessageGroup:
    ...
    def __init__(self, message_type: MessageType, me_messages: List[MEMessage], max_messages_per_user=10,
                 max_messages_per_server=100, delete_on_startup=False, ephemeral=False):
    ...
    def purge_user_messages(self, user_id: int, server_id: int, max_messages_per_user: int = None):
        ...
        message_df["first_message_id"] = message_df["first_message_id"].astype(int)
    ...
    def purge_server_messages(self, server_id: int, max_messages_per_server: int = None):
        pass
        # TODO Finish/Test this
        # if max_messages_per_server is None:
        #     max_messages_per_server = self.max_messages_per_server
        # message_df = self.get_db().get_messages_of_type_and_server_df(self.message_type.value, server_id)
        # message_df = message_df.drop_duplicates(['first_message_id', 'channel_id', 'server_id'])
        # message_and_channel_ids = message_df.values.tolist()
    ...
    async def display(self, user_id: int=None, channel: int = None,
        ...
        if len(messages) > 0 and not ephemeral:  # Don't save ephemeral messages, they might not be accurate
            ...
            self.purge_user_messages(user_id, messages[0].guild.id)
            self.purge_server_messages(messages[0].guild.id)
